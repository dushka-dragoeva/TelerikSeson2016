namespace Cars.Tests.JustMock.Mocks
{
    ...
    public class MoqCarsRepository : CarRepositoryMock, ICarsRepositoryMock
    {
        protected override void ArrangeCarsRepositoryMock()
        {
            ...
            mockedCarsRepository.Setup(r => r.Search(It.Is<string>(x => x == string.Empty))).Returns(this.FakeCarCollection.ToList());
            mockedCarsRepository.Setup(r => r.GetById(It.IsAny<int>())).Returns(this.FakeCarCollection.First());
            mockedCarsRepository.Setup(r => r.GetById(It.Is<int>(x => x == -1))).Returns((Car)null);
