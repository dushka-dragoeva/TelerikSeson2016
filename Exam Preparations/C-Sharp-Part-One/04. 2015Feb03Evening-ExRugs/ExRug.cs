        ...
        for (int row = 0; row < rugSize; row++)
        {
            for (int col = 0; col < rugSize; col++)
            {
                ...
                /// print dots
                    ...
                    || (row > middleRug + middlePattern && row - col > middlePattern && row + col > rugSize - 1 + middlePattern)
                    || (row + col < rugSize - 1 - middlePattern && row - col > middlePattern)
                    || (row + col > rugSize - 1 + middlePattern && col - row > middlePattern))
                {
                    Console.Write(dot);
                }
                else
