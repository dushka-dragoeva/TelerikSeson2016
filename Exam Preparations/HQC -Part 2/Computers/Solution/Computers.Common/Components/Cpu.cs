namespace Computers.Common.Components
{
    ...
    public abstract class Cpu : MotherboardComponent, ICpu
    {
        ...
        public void SquareNumber()
        {
            ...
            if (data < 0)
            {
                base.Motherboard.DrawOnVideoCard(NumberTooLowMessage);
            }
            else if (data > this.GetMaxNumber())
            {
                Motherboard.DrawOnVideoCard(NumberTooHighMessage);
            }
            else
            {
                var value = data * data;
                Motherboard.DrawOnVideoCard(string.Format(SquareOfMessage, data, value));
            }
        }
